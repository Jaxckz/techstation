...
export type UserRole = 'ADMIN' | 'ENGINEER' | 'SECURITY' | 'OA_SPECIALIST';
...
export interface AuditLogEntry {
  id?: number;
  username: string;
  action: 'LOGIN' | 'LOGOUT' | 'DELETE_DATA' | 'UPDATE_SETTINGS' | 'EXPORT_DATA' | 'USER_MANAGEMENT';
  details: string;
  ip: string;
  timestamp: string; // Server returns string date
}
...
export type OAEventType = 'release' | 'issue' | 'update' | 'request';
...
export interface OAEvent {
  ...
  version?: string;
  content: string;
  status: '待处理' | '已处理' | 'pending' | 'resolved' | 'deployed' | 'closed' | string; // Support custom strings
  date: string;
  author: string;
  createdAt: Date;
  attachments: string[];
